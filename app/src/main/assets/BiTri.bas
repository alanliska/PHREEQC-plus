...
OPEN "I", #40, "/data/data/cz.p/files/thermo_s_P3.txt"
...
INPUT #40, ThermoP3$
LenghtDataP3% = LEN(ThermoP3$)
Lim130% = INSTR(ThermoP3$, " ")
NameP3$ = MID$(ThermoP3$, 1, Lim130%-1)
Lim131% = INSTR(ThermoP3$, " ", Lim130%+1)
FormulaP3$ = MID$(ThermoP3$, Lim130%+1, Lim131%-Lim130%-1)
Lim132% = INSTR(ThermoP3$, " ", Lim131%+1)
MethodP3$ = MID$(ThermoP3$, Lim131%+1, Lim132%-Lim131%-1)
Lim133% = INSTR(ThermoP3$, " ", Lim132%+1)
TotDummyP3$ = MID$(ThermoP3$, Lim132%+1, Lim133%-Lim132%-1)
Lim134% = INSTR(ThermoP3$, " ", Lim133%+1)
HP3$ = MID$(ThermoP3$, Lim133%+1, Lim134%-Lim133%-1)
Lim135% = INSTR(ThermoP3$, " ", Lim134%+1)
Dummy1P3$ = MID$(ThermoP3$, Lim134%+1, Lim135%-Lim134%-1)
Lim136% = INSTR(ThermoP3$, " ", Lim135%+1)
Dummy2P3$ = MID$(ThermoP3$, Lim135%+1, Lim136%-Lim135%-1)
Lim137% = INSTR(ThermoP3$, " ", Lim136%+1)
SP3$ = MID$(ThermoP3$, Lim136%+1, LenghtDataP3%-Lim137%)
SP3 = VAL(SP3$)
HP3 = VAL(HP3$)
...
IF TALLY(FormulaP3$,"+")>0 THEN
LenP3% = LEN(FormulaP3$)
PosP3% = INSTR(FormulaP3$, "+")
ChargeCharacterP3$ = MID$(FormulaP3$, PosP3%, LenP3%-PosP3%+1)
SMS_P3$ = CHR$(091)+FormulaP3$+CHR$(093)
FormulaP3$ = CHR$(091)+FormulaP3$+CHR$(093)+ChargeCharacterP3$
ELSE IF TALLY(FormulaP3$,"-")>0 THEN
LenP3% = LEN(FormulaP3$)
PosP3% = INSTR(FormulaP3$, "-")
ChargeCharacterP3$ = MID$(FormulaP3$, PosP3%, LenP3%-PosP3%+1)
SMS_P3$ = CHR$(091)+FormulaP3$+CHR$(093)
...
ELSE
SMS_P3$ = CHR$(091)+FormulaP3$+CHR$(093)
FormulaP3$ = CHR$(091)+FormulaP3$+CHR$(093)
ENDIF
...
deltaHact_b = HTS - HP1 - HP2 - HP3
deltaSact_f = STS - SR1 - SR2
deltaSact_b = STS - SP1 - SP2 - SP3
...
PRINT #7, SMS_P3$, FormulaP3$, "0", SMS_P3$, 1

PRINT #8, FormulaR1$;" = ";FormulaR1$
PRINT #8, FormulaR2$;" = ";FormulaR2$
PRINT #8, FormulaP1$;" = ";FormulaP1$
PRINT #8, FormulaP2$;" = ";FormulaP2$
PRINT #8, FormulaP3$;" = ";FormulaP3$

INPUT #11, DatasetName$

PRINT #9, DatasetName$;"_forward ; -formula ";FormulaR1$;" -1 ";FormulaR2$;" -1 ";FormulaP1$;" 1 ";FormulaP2$;" 1 ";FormulaP3$;" 1 "
PRINT #9, DatasetName$;"_backward ; -formula ";FormulaP1$;" -1 ";FormulaP2$;" -1 ";FormulaP3$;" -1 ";FormulaR1$;" 1 ";FormulaR2$;" 1 "
...
PRINT #10, DatasetName$;"_backward ; -start ; 10 if (MOL("+CHR$(034)+FormulaP1$+CHR$(034)+") <= 1e-15) then goto 200 ; 20 if (MOL("+CHR$(034)+FormulaP2$+CHR$(034)+") <= 1e-15) then goto 200 ; 30 if (MOL("+CHR$(034)+FormulaP3$+CHR$(034)+") <= 1e-15) then goto 200 ; 100 moles = "+STR$(k_backward)+" "+CHR$(042)+" MOL("+CHR$(034)+FormulaP1$+CHR$(034)+") "+CHR$(042)+" MOL("+CHR$(034)+FormulaP2$+CHR$(034)+") "+CHR$(042)+" MOL("+CHR$(034)+FormulaP3$+CHR$(034)+") "+CHR$(042)+" TIME ; 200 SAVE moles ; -end"
