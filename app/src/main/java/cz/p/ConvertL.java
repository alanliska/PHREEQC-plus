...
public class ConvertL extends MainActivity {
    ...
    public void onStart()
    {
        ...
        output3(exec("cat "+getFilesDir()+"/Thermochemistry_l.txt"));
    }
...
//                    }
                ...
                InputData = InputData.replace("[C][H][N][O][S]Z", "CHNOSZ");
//                } catch (Exception e) {
//                    e.printStackTrace();
//                }
                /////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//                startActivity(intent);
    ...
    public void makeDatabase_l(){
        try {
            exec(getApplicationInfo().nativeLibraryDir+"/libxbbc.so -o "+getFilesDir()+"/LIQUIDS/DatabaseMakerPseudoPhases_l.b "+getFilesDir()+"/LIQUIDS/DatabaseMakerPseudoPhases_l.bas");
            exec("chmod -R 755 "+getFilesDir()+"/LIQUIDS/DatabaseMakerPseudoPhases_l.b");
            exec(getApplicationInfo().nativeLibraryDir+"/libxbvm.so "+getFilesDir()+"/LIQUIDS/DatabaseMakerPseudoPhases_l.b");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private void modifyOutput_l() {
        try {
            String Raw_g = exec("cat "+getFilesDir()+"/LIQUIDS/Database_l.dat");
	    ...
	    while (Raw_g.contains("=  + e- =")){  //2 spaces
            ...
            FileOutputStream fileout115 = openFileOutput("Database_l1.dat",MODE_PRIVATE);
            OutputStreamWriter outputWriter115 = new OutputStreamWriter(fileout115);
            outputWriter115.write(Raw_g);
            outputWriter115.close();

            String Raw_g2 = exec("cat "+getFilesDir()+"/Database_l1.dat");
            while (Raw_g2.contains("(l) ;  = ")){  //2 spaces
                Raw_g2 = Raw_g2.replace("(l) ;  = ", ""); //(2 spaces, 1 space)
            }
            FileOutputStream fileout215 = openFileOutput("Database_l2.dat",MODE_PRIVATE);
        ...
        try {
            String Fastchem_database_content = exec("cat "+getFilesDir()+"/LIQUIDS/Fastchem_l.dat");
...
//            Fastchem_database_content = Fastchem_database_content.replace("[F]", "F");
        ...
        exec("rm "+getFilesDir()+"/LIQUIDS/Fastchem_l.dat");
        exec("mv "+getFilesDir()+"/Fastchem_l.tmp "+getFilesDir()+"/LIQUIDS/Fastchem_l.dat");
    ...
    protected void onResume() {
        ...
        output2(exec("cat "+getFilesDir()+"/dataset-name.txt"));
        output3(exec("cat "+getFilesDir()+"/Thermochemistry_l.txt"));
        outputX(exec("cat "+getFilesDir()+"/Database_l2.dat"));
    }
